...
func beginGitOperation(root string, items []skill) (*operationRecord, error) {
	...
	if !within(canonicalLocation(root), canonicalLocation(gitDir)) {
		paths = append(paths, gitDir)
		common, err := gitOutput(root, "rev-parse", "--path-format=absolute", "--git-common-dir")
		if err != nil {
			return nil, err
		}
		branch, err := gitOutput(root, "symbolic-ref", "HEAD")
		if err != nil {
			return nil, err
		}
	...
	files, err := gitOutput(root, "diff", "--name-only", "--no-ext-diff", "HEAD", "@{upstream}", "--")
	if err != nil {
		return nil, err
	}
...
func refreshExternalCatalog(operation *operationRecord) error {
	...
	for i := range catalog.Packages {
		...
		for _, step := range operation.Steps {
			if samePath(step.Path, pkg.Directory) || within(step.Path, pkg.Directory) {
				touched = true
				break
			}
		}
		...
		digest, err := hashDirectory(pkg.Directory)
		if err != nil {
			return err
		}
		if digest != pkg.Digest {
			...
			for _, binding := range pkg.Bindings {
				...
				before, err := hashDirectory(binding.Path)
				...
				if after, err := hashDirectory(binding.Path); err != nil || after != digest {
...
func pathsCover(paths []string, target string) bool {
	for _, path := range paths {
		if within(canonicalLocation(path), canonicalLocation(target)) {
			return true
		}
	}
	return false
