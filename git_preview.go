...
func previewRepository(ctx context.Context, timeout time.Duration, repo *repository, stdout, stderr io.Writer) bool {
	...
	merge, err := gitOutput(repo.Root, "config", "--get", "branch."+branch+".merge")
	if err != nil {
		return fail(err)
	}
	source, err := gitOutput(repo.Root, "remote", "get-url", remote)
	if err != nil {
		return fail(err)
	}
	...
	dirty, err := gitOutput(repo.Root, "status", "--porcelain")
	if err != nil {
		return fail(err)
	}
	...
	if _, err := gitNetworkOutputWithTimeout(ctx, timeout, temporary, "clone", "--bare", "--no-hardlinks", "--", repo.Root, bare); err != nil {
		return fail(err)
	}
	if _, err := gitNetworkOutputWithTimeout(ctx, timeout, bare, "fetch", "--no-tags", "--no-recurse-submodules", "--", source, merge); err != nil {
		return fail(err)
	}
	counts, err := gitOutput(bare, "rev-list", "--left-right", "--count", head+"...FETCH_HEAD")
	if err != nil {
		return fail(err)
	}
