...
// Parse accepts flags before or after skill names and returns exit code 2 for invalid arguments.
func Parse(args []string, stderr io.Writer) (Options, int) {
	...
	for len(args) > 0 {
		...
		case "--host", "--agent", "-a":
			opt.Hosts = append(opt.Hosts, NormalizeHost(value))
		case "--scope":
...
func Validate(command string, opt Options) error {
	...
	if opt.DryRun && !IsWrite(command) && command != "check" && command != "track" && !(command == "doctor" && opt.Fix) {
...
// IsWrite identifies command families that can mutate state; callers apply dry-run and subcommand rules.
...
// NormalizeHost maps supported Agent aliases to their canonical names.
