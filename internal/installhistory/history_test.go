...
func TestTrustedCommandsAcceptsEscapedToolNames(t *testing.T) {
	for _, name := range []string{`Bash`, `bash`, `\u0042ash`, `ba\u0073h`} {
		line := []byte(`{"type":"assistant","message":{"content":[{"type":"tool_use","name":"` + name + `","input":{"command":"npx skills add owner/repo"}}]}}`)
		if got := trustedCommands(line); len(got) != 1 || got[0] != "npx skills add owner/repo" {
			t.Fatalf("tool %q: %v", name, got)
		}
	}
	line := []byte(`{"payload":{"arguments":"{\"cmd\":\"npx skills add owner/repo\"}","name":"\u0065xec_command","type":"function_call"},"type":"response_item"}`)
	if got := trustedCommands(line); len(got) != 1 {
		t.Fatalf("escaped Codex tool: %v", got)
	}
}

func TestReadHistoryBufferBoundaries(t *testing.T) {
	root := t.TempDir()
	var content strings.Builder
	for i, size := range []int{0, 64 * 1024, 128*1024 + 1, 256 * 1024} {
		line := `{"type":"assistant","padding":"` + strings.Repeat("x", size) + `","message":{"content":[{"type":"tool_use","name":"Bash","input":{"command":"npx skills add owner/repo --skill sample-` + string(rune('a'+i)) + `"}}]}}`
		content.WriteString(line)
		if i != 3 { // The final valid record deliberately has no newline.
			content.WriteByte('\n')
		}
	}
	if err := os.WriteFile(filepath.Join(root, "records.jsonl"), []byte(content.String()), 0600); err != nil {
		t.Fatal(err)
	}
	got, err := ReadRoots([]string{root})
	if err != nil || len(got) != 4 {
		t.Fatalf("buffer boundaries lost records: %v %v", got, err)
	}
}
...
func TestReadInstallHistoryRoots(t *testing.T) {
	...
	got, err := ReadRoots([]string{root, filepath.Join(root, "missing")})
	if err != nil {
		t.Fatal(err)
	}
