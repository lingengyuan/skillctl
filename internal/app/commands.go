...
func runManager(ctx context.Context, command string, opt options, stdout, stderr io.Writer) int {
	...
	case "version":
		if opt.JSON {
			return outputResult(commandResult{Command: command, Result: map[string]string{"version": opt.version}}, opt, stdout, stderr, false)
		}
	...
	if !opt.DryRun && (items != nil || slices.Contains([]string{"install", "sync", "import", "update"}, plan.Command)) {
		...
		for _, pkg := range catalog.Packages {
			...
			for _, prior := range items {
				affected = affected || prior.ID == pkg.ID || fsutil.SamePath(prior.Path, pkg.Directory)
			}
			for _, change := range plan.mutations {
				affected = affected || fsutil.SamePath(change.Path, pkg.Directory)
				for _, binding := range pkg.Bindings {
					affected = affected || fsutil.SamePath(change.Path, binding.Path)
				}
			}
			if affected {
