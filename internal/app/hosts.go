...
func augmentRoots(roots []scanRoot, opt options) []scanRoot {
	...
	if opt.ConfigPath == "" || opt.Project != "" {
		...
		if cwd, err := os.Getwd(); err == nil && opt.Project == "" && fsutil.Within(project, cwd) {
			for dir := cwd; !fsutil.SamePath(dir, project); dir = filepath.Dir(dir) {
	...
	for i := range roots {
		roots[i].Host = cli.NormalizeHost(roots[i].Host)
	}
	return roots
}
