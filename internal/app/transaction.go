...
func mutation(path, kind string) (pathMutation, error) {
	...
	absolute = fsutil.BindingPath(absolute)
...
func prepareOperation(command string, changes []pathMutation, affected []string) (*operationRecord, error) {
	...
	r := &operationRecord{SchemaVersion: 1, ID: id, Command: command, Created: time.Now().UTC(), State: "preparing", Affected: affected, directory: fsutil.BindingPath(filepath.Join(dir, id))}
	for i, change := range changes {
		if fsutil.Within(change.Path, r.directory) || fsutil.Within(r.directory, change.Path) {
		...
		for _, earlier := range changes[:i] {
			if fsutil.Within(earlier.Path, change.Path) || fsutil.Within(change.Path, earlier.Path) {
	...
	for i, change := range changes {
		...
		case "directory":
			if change.ContentDigest != "" {
				if err := fsutil.CopyDirectory(change.Source, afterPath); err != nil {
					return nil, err
				}
				if digest, err := fsutil.HashDirectory(afterPath); err != nil || digest != change.ContentDigest {
