...
func TestUpdateAppliesTimeoutPerProviderOperation(t *testing.T) {
	synctest.Test(t, func(t *testing.T) {
		...
		for _, name := range []string{"first-skill", "second-skill"} {
			...
			installedHash, err := fsutil.HashDirectory(installed[name])
			if err != nil {
				t.Fatal(err)
			}
			remote := filepath.Join(sourceRoot, name)
			writeTestSkill(t, remote, name, "new")
			remoteHashes[name], err = fsutil.HashDirectory(remote)
			if err != nil {
				t.Fatal(err)
			}
		...
		for name, installedPath := range installed {
			got, err := fsutil.HashDirectory(installedPath)
			if err != nil {
				t.Fatal(err)
			}
...
func newTrackedFixture(t *testing.T) (skill, *trackedState, *sourceSession, string) {
	...
	installedHash, err := fsutil.HashDirectory(installed)
	if err != nil {
		t.Fatal(err)
	}
...
// Tests exercise the same application entry used by the executable.
func run(args []string, stdout, stderr io.Writer) int {
	return Run(context.Background(), args, "test", stdout, stderr)
}
