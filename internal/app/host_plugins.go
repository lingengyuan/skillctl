...
func discoverClaudePlugins(project string) ([]hostPlugin, []diagnostic) {
	...
	for id, records := range index.Plugins {
		for _, record := range records {
			if record.Scope != "user" && record.Scope != "managed" && (project == "" || !fsutil.SamePath(project, record.ProjectPath)) {
				continue
			}
...
func pluginRoots(plugins []hostPlugin) ([]scanRoot, []managedRoot, []diagnostic) {
	...
	for _, plugin := range plugins {
		...
		for _, path := range paths {
			...
			if filepath.IsAbs(path) || !fsutil.Within(plugin.Directory, root) {
...
func applyPluginBaselines(plugins []hostPlugin, persist bool) ([]hostPlugin, []diagnostic) {
	...
	for i := range plugins {
		...
		if persist {
			if digest, err := fsutil.HashDirectory(plugin.Directory); err == nil {
