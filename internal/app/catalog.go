...
func sourceIdentity(source sourceSpec) string {
	return "skill-" + stableID(source.Kind, gitstore.NormalizeSource(source.URL), filepath.ToSlash(source.SkillPath), source.Ref)
}
...
func loadCatalog() (*packageCatalog, error) {
	...
	for _, pkg := range catalog.Packages {
		if pkg.ID == "" || seen[pkg.ID] || !filepath.IsAbs(pkg.Directory) || !skilldoc.ValidName(pkg.Name) {
...
func (c *packageCatalog) byPath(path string) *managedPackage {
	for i := range c.Packages {
		pkg := &c.Packages[i]
		if fsutil.SamePath(pkg.Directory, path) {
			return pkg
		}
		for _, binding := range pkg.Bindings {
			if fsutil.SamePath(binding.Path, path) || (binding.DisabledPath != "" && fsutil.SamePath(binding.DisabledPath, path)) {
				return pkg
			}
		}
	}
