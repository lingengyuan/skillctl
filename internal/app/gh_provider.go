...
func readGHSkillClaim(item skill) ghSkillClaimResult {
	...
	claim := ghSkillClaim{
		Repository: skilldoc.MetadataString(document.Metadata, "github-repo"),
		Ref:        skilldoc.MetadataString(document.Metadata, "github-ref"),
		TreeSHA:    skilldoc.MetadataString(document.Metadata, "github-tree-sha"),
		SkillPath:  skilldoc.MetadataString(document.Metadata, "github-path"),
		LocalPath:  skilldoc.MetadataString(document.Metadata, "local-path"),
		Pinned:     skilldoc.MetadataBool(document.Metadata, "github-pinned"),
	}
...
func updateGHSkillProviderNative(ctx context.Context, session *sourceSession, item skill, claim ghSkillClaim, progress io.Writer) (ghSkillClaim, error) {
	...
	if err == nil {
		...
		if !result.Found || result.Err != nil {
			...
			name, readErr := skilldoc.ReadName(filepath.Join(item.Path, "SKILL.md"))
...
func createDirectorySnapshot(installed string) (*directorySnapshot, error) {
	...
	if err := fsutil.CopyDirectory(installed, directory); err != nil {
...
func (s *directorySnapshot) restore(installed string) error {
	if _, err := os.Lstat(installed); os.IsNotExist(err) {
		...
		if err := fsutil.CopyDirectory(s.directory, stage); err != nil {
			return err
		}
