...
func (p *lifecyclePlan) add(change pathMutation) error {
	for _, old := range p.mutations {
		if samePath(old.Path, change.Path) {
...
func targetBindings(opt options, roots []scanRoot, name string) ([]skillBinding, error) {
	...
	for _, host := range hosts {
		...
		for _, root := range roots {
			if root.Host == host && root.Scope == scope && (scope != "project" || root.Project == "" || samePath(root.Project, project)) {
		...
		for _, candidate := range candidates {
			if samePath(candidate.Path, preferred) {
				root = candidate.Path
				break
			}
		}
		...
		for _, candidate := range roots {
			if samePath(candidate.Path, root) && candidate.Host != host && candidate.Scope == scope {
...
func planInstallPackage(plan *lifecyclePlan, catalog *packageCatalog, prepared preparedPackage, bindings []skillBinding, environment string) error {
	...
	for _, host := range agentHosts() {
		if within(physicalLocation(host.UserPath), physicalLocation(directory)) {
			return fmt.Errorf("shared store must be outside Agent skill directories; move SKILLCTL_HOME")
		}
	}
	for _, binding := range bindings {
		if within(physicalLocation(filepath.Dir(binding.Path)), physicalLocation(directory)) {
			return fmt.Errorf("shared store must be outside Agent skill directories; move SKILLCTL_HOME")
		}
	}
	if err != nil {
	...
	if hash, err := hashDirectory(directory); err == nil {
	...
	for _, binding := range bindings {
		index := slices.IndexFunc(pkg.Bindings, func(b managedBinding) bool {
			return samePath(b.Path, binding.Path) && b.Host == binding.Host && b.Scope == binding.Scope
		})
		...
		if !needsChange {
			...
			for _, known := range pkg.Bindings {
				if samePath(known.Path, binding.Path) && known.Enabled {
					compatible = true
				}
			}
			if !compatible {
				...
				for _, other := range previous.Bindings {
					if !samePath(other.Path, binding.Path) {
						continue
					}
...
func planBindingChange(command string, view *inventoryView, selected []skillAsset, opt options) (*lifecyclePlan, error) {
	...
	for _, asset := range selected {
		...
		for i := range pkg.Bindings {
			...
			if command != "enable" {
				for _, other := range pkg.Bindings {
					if other.Enabled && !bindingMatches(other.skillBinding, opt) && (samePath(other.Path, binding.Path) || samePath(binding.Path, pkg.Directory)) {
		...
		if command == "remove" {
			for _, binding := range pkg.Bindings {
				if bindingMatches(binding.skillBinding, opt) && samePath(binding.Path, pkg.Directory) {
...
func planPackageUpdate(plan *lifecyclePlan, pkg *managedPackage, prepared preparedPackage) error {
	...
	if change.Expected == "missing" {
		...
		if hash, err := hashDirectory(directory); err != nil || hash != prepared.Digest {
