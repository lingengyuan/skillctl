...
func validateEnvironment(manifest *environmentManifest) error {
	...
	for i := range manifest.Skills {
		...
		if !safeComponent(item.ID) || seen[item.ID] || !skillName.MatchString(item.Name) || item.Source == "" {
		...
		for j, host := range item.Hosts {
			item.Hosts[j] = canonicalHost(host)
		}
		if item.Mode == "" {
			item.Mode = "link"
		}
		...
		if filepath.IsAbs(item.SkillPath) || strings.Contains(item.SkillPath, "\\") || !within("/source", filepath.Join("/source", item.SkillPath)) {
...
func preparedLocked(ctx context.Context, entry lockedSkill, item desiredSkill, base string, offline bool) (preparedPackage, func(), error) {
	...
	if resolved.Kind == "well-known" {
		...
		if err := unpackArtifact(body, dir); err != nil {
			cleanup()
...
func activeProfilePath(path string) (string, error) {
	return stateFile(filepath.Join("profiles", stableID(canonicalPathKey(path))+".json"))
}
...
func runManifestSync(ctx context.Context, command string, opt options, stdout, stderr io.Writer) int {
	...
	environment := stableID(canonicalPathKey(path))
	...
	for _, item := range manifest.Skills {
		...
		for i := range pkg.Bindings {
			...
			for _, desired := range bindings {
				if binding.Host == desired.Host && samePath(binding.Path, desired.Path) {
	...
	for p := range view.catalog.Packages {
		...
		for b := range pkg.Bindings {
			...
			for _, other := range pkg.Bindings {
				if other.Enabled && samePath(other.Path, binding.Path) && (other.Manual || len(other.Environments) > 0) && other.Host != binding.Host {
