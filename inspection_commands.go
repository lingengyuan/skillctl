...
func runInspection(ctx context.Context, view *inventoryView, selected []skillAsset, command string, opt options, stdout, stderr io.Writer) int {
	...
	if !opt.NoHistory && !opt.Offline {
		...
		trackFromInstallHistory(ctx, view.timeout, pending, view.state, view.manifests, view.managed, false, io.Discard, &evidenceErrors)
	...
	if len(legacy) > 0 {
		...
		for _, r := range legacyReports {
			if command == "update" && opt.DryRun && r.UpdateAvailable && r.Drift != "modified" {
				...
				for _, asset := range selected {
					if samePath(asset.Path, r.Path) || r.Provider == "git-worktree" && within(unit, asset.Path) {
			...
			for i := range selected {
				if samePath(selected[i].Path, r.Path) {
			...
			if err != nil {
				...
				for i, asset := range selected {
					for _, current := range fresh.Items {
						if samePath(current.Path, asset.Path) || current.ID == asset.ID && current.ContentID == asset.ContentID {
							current.State, current.ReasonCode, current.Error = asset.State, asset.ReasonCode, asset.Error
							selected[i] = current
							for j := range reports {
								if samePath(reports[j].Path, asset.Path) {
